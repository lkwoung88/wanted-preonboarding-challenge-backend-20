...
사용자간 거래에서 결제플랫폼을 통해 결제를 진행합니다. 3-1. 이 과정에서 결제 대행사는 임의로 설정하셔도 괜찮습니다.
결제진행, 가상계좌, 결제취소 기능을 개발해주세요. (변경: 계좌이체 -> 가상계좌)
결제연동문서를 자세히 보고 단순 기능 추가가 아닌 실제 결제가 진행된다는 관점에서 기능을 구현해주세요.
다른 참여자분들의 PR을 읽어보면서 의견을 주고 받아주세요.
개발 과정에서 어려웠던 부분이 있었다면, 기록을 남겨주세요.
업데이트
